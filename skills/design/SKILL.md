...
- **Don't ask for confirmation on obvious fixes.** When the navigator flags an error and the agent can see the same error after visual inspection, fix it immediately. Don't burn a turn asking "shall I fix this?" — the navigator already told you it's broken and you agree. Confirmation rounds are for creative/directional decisions, not for fixing glitches that both parties can see. Asking to confirm an obvious repair is a bad experience — it serializes a round-trip for no decision value.
- **Don't reposition non-symmetric shapes without redrawing.** If a shape isn't rotationally symmetric (parallelograms, isometric step polygons), changing its orientation means the geometry changes — repositioning is not the same as reorienting. Analyze whether existing components can be reused in the new orientation BEFORE placing them on canvas. If the shape's face angles are coupled to its orientation, create a new component for the new direction.
- **Don't iterate blind — draw the grid.** When stuck on projection, alignment, or spatial constraint problems, draw the projection grid lines first to reveal what's possible. The grid makes constraints visible that iteration at the same fidelity never reveals. Don't spend multiple rounds guessing — one grid frame answers the question.
